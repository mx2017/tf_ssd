...
def _draw_box(im, box_list, label_list, color=(0,255,0), cdict=None, form='center'):
  ...
  for bbox, label in zip(box_list, label_list):
    ...
    cv2.putText(im, label, (xmin, ymax), font, 0.8, c, 2)
...
def print_config_info(mc):
    print ("===========[print_config_info]==================")
    print ("pretrained_model_path:")
    print ("        ",FLAGS.pretrained_model_path)
    if mc.B_CONF_LOSS_V1 == True:
      print ("mc.CONF_LOSS_GAMMA:",mc.CONF_LOSS_GAMMA)
      print ("mc.CONF_LOSS_X_OFFSET:",mc.CONF_LOSS_X_OFFSET)
      print ("mc.CONF_LOSS_Y_OFFSET:",mc.CONF_LOSS_Y_OFFSET)
  
        
def filter_variables(all_variables):
    left_variables = []

    filter_key = ['Momentum','iou','global_step','conv_predict','conv12',
      'Conv2d_15_','Conv2d_14_']
    #'Conv2d_12_depthwise',
...
def filter_param(all_variables):
    left_variables = []

    filter_key = ['Momentum','iou','global_step','fire12','fire13','conv12']
    for v in all_variables:
    #  print ("name:",v.name)
...
#训练mobileDet，因为第一次要用imagenet1000的mobilenet模型作为预训练模型，
# 第一次设置为False,训练暂停再重启设置为True
resume = True

#用squeezedetv1的训练好的模型作为预训练模型,一般是用来做量化的
load_param = False


def record_train_info(step):
    pass
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.dataset == 'KITTI':
      if FLAGS.net == 'vgg16':
        ...
        mc.PRETRAINED_MODEL_PATH = FLAGS.pretrained_model_path
        mc.is_training = True
        print ('pretrain:',mc.PRETRAINED_MODEL_PATH)
    ...
    if FLAGS.net == 'mobileDet' or FLAGS.net == 'mobileDet_V1_025':
        ...
        if load_param == True:
            param_filter = filter_param(model.model_params)
            saver_param = tf.train.Saver(param_filter)
        saver = tf.train.Saver(tf.global_variables())
    ...
    MODEL_SAVE_PATH = FLAGS.train_dir + '/../model_save'
    ckpt = tf.train.get_checkpoint_state(FLAGS.restore_dir)
    print ("===>FLAGS.restore_dir:",FLAGS.restore_dir)
    
    #ckpt = tf.train.get_checkpoint_state("/disk3/tf_workspace/squeezeDet-master/data/model_checkpoints/squeezeDet1")
    ...
    if ckpt and ckpt.model_checkpoint_path:
        if FLAGS.net == 'mobileDet' or FLAGS.net == 'mobileDet_V1_025':
            ...
            if load_param == True:  
                print ('======>restore ',ckpt.model_checkpoint_path,' load parameter')
                ret = saver_param.restore(sess, ckpt.model_checkpoint_path)
            else:
                if resume == True:
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      if step % FLAGS.summary_step == 0:
        ...
        op_list = [
            ...
            model.bbox_loss, model.class_loss,model.lr,
            model.conf_loss_positive,model.conf_loss_negtive
        ]
        _, loss_value, summary_str, det_boxes, det_probs, det_class, \
            conf_loss, bbox_loss, class_loss,lr,pos_loss,neg_loss = sess.run(
        ...
        if mc.NUM_THREAD > 0:
          _, loss_value, conf_loss, bbox_loss, class_loss,lr,pos_loss,neg_loss = sess.run(
              [model.train_op, model.loss, model.conf_loss, model.bbox_loss,
               model.class_loss,model.lr,model.conf_loss_positive, model.conf_loss_negtive], 
                options=run_options)
        else:
      ...
      if step % 10 == 0:
        ...
        format_str = ('%s: step %d, loss=%.2f lr=%.8f cls_loss=%.6f pos_loss=%.6f neg_loss=%.6f(%.1f images/sec; %.3f '
                      'sec/batch)')
        print (format_str % (datetime.now(), step, loss_value,lr,class_loss,pos_loss,neg_loss,
