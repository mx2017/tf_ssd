...
class ModelSkeleton:
  ...
  def _sigmoid_loss(self,x):
    ...
    #return tf.clip_by_value(1.0/(1.0+tf.exp(-GAMMA*(x+X_OFFSET))) + Y_OFFSET,0,1)
    #return tf.clip_by_value(1.0/(1.0+tf.exp(-12*(x-0.40))),0,1)
    #return tf.clip_by_value(1.0/(1.0+tf.exp(-8*(x-0.43))),0,1)
  ...
  '''
  def depthwise_kernel_mutiply_max(self,depthwise_kernel,max):
    input_channels = int(depthwise_kernel.get_shape()[-2])
    kernel_groups = tf.split(axis=2, num_or_size_splits=input_channels, value=depthwise_kernel)
    max_groups = tf.split(axis=0, num_or_size_splits=input_channels, value=max)
    kernel_1 = [i*j for i,j in zip(kernel_groups,max_groups)]
    kernel_output = tf.concat(axis=3, values=kernel_1)

    #print ("==========>kernel_output.shape:",kernel_output.get_shape())
    return kernel_output
  '''
  
  def depthwise_quant_activations(self,mc,act):
    input_channels = int(act.get_shape()[-1])

    quant_act = lambda i : self._quant_activations(mc,i)
    
    act_groups = tf.split(axis=3, num_or_size_splits=input_channels, value=act)
    act_output = [quant_act(i) for i in act_groups]
    
    outputs = tf.concat(axis=3, values=act_output)

    return  outputs

    
  def depthwise_quant_kernel(self,mc,depthwise_kernel):
      
    input_channels = int(depthwise_kernel.get_shape()[-2])

    quant_kenel = lambda i : self._quant_kernel_v1(mc,i)
    
    kernel_groups = tf.split(axis=2, num_or_size_splits=input_channels, value=depthwise_kernel)
    kernel_output = [quant_kenel(i) for i in kernel_groups]
    
    outputs = tf.concat(axis=2, values=kernel_output)
    
    return  outputs
    
  def my_depthwise_conv2d(self,mc,inputs, depthwise_kernel,stride,padding,name=None):
    ...
    if groups == 1:
        conv = convolve(inputs, depthwise_kernel)

    # In the cases of multiple groups, split inputs & weights and
    else:
        # Split input and weights and convolve them separately
        input_groups = tf.split(axis=3, num_or_size_splits=groups, value=inputs)
        weight_groups = tf.split(axis=2, num_or_size_splits=groups,
  ...
  def _depthwise_separable_conv(self, inputs, layer_name,filters,kernel_size, 
                stride,relu=True,width_multiplier=1,depth_multiplier=1,
          conv_with_bias=False,stddev=0.001,use_batchnorm=True,freeze=False,activation_fn=tf.nn.relu):
     
      depthwise_conv = self._conv_depthwise_layer(inputs,layer_name,filters,kernel_size,stride,
                                relu=True,activation_fn=activation_fn,freeze=freeze)

      self._add_act(depthwise_conv,layer_name+'_depthwise')

      depthwise_num_outputs = width_multiplier * filters
      out = self._conv_bn_layer(layer_name+'_pointwise',depthwise_conv,filters=depthwise_num_outputs,
                              relu=True,activation_fn=activation_fn,kernel_name='weights',
                                  freeze=freeze,size=1,stride=1) 
                                  
      #out = self._conv_pointwise_layer(depthwise_conv,layer_name+'_pointwise',
      #                     depthwise_num_outputs,freeze=freeze)
      self._add_act(out,layer_name+'_pointwise')
      
      return out
          
  
  #generally if threre exists batchnorm,then it's will not necessery to add bias.
  def _conv_depthwise_layer(
      self, inputs, layer_name,filters,kernel_size, stride,relu=True,activation_fn=tf.nn.relu,width_multiplier=1,depth_multiplier=1,
          conv_with_bias=False,stddev=0.001,use_batchnorm=True,freeze=False):

    mc = self.mc
    with tf.variable_scope(layer_name+'_depthwise') as scope:
      in_channels = inputs.get_shape()[3]

      depthwise_kernel_init = tf.truncated_normal_initializer(
          stddev=stddev, dtype=tf.float32)
      
      depthwise_kernel = _variable_with_weight_decay(
          'depthwise_weights', shape=[kernel_size, kernel_size, int(in_channels), depth_multiplier],
          wd=mc.WEIGHT_DECAY, initializer=depthwise_kernel_init, trainable=(not freeze))
 
      
      '''
      depthwise_conv = self.my_depthwise_conv2d(mc,inputs, depthwise_kernel, 
                                   stride,padding='SAME',name=layer_name+'_depthwise')
      '''
      
      #add quant
      #if mc.QuantVersion == 'V1':
      #  depthwise_kernel = self.depthwise_quant_kernel(mc,depthwise_kernel)

      '''
      if mc.bQuant == True:
        if mc.bQuantWeights == True:
          depthwise_kernel = self._quant_kernel_v1(mc,depthwise_kernel)
      '''
      ...
      if mc.bQuant == True:
        if mc.bQuantActivations == True:
          inputs = self.depthwise_quant_activations(mc,inputs)
      '''
    
      depthwise_conv = tf.nn.depthwise_conv2d(inputs, depthwise_kernel, 
                                      [1, stride, stride, 1],padding='SAME',name=layer_name+'_depthwise')      

      self._add_act(depthwise_conv,'verify_' + layer_name+'_depthwise')
      
      #print ('in_channels:',in_channels)
      depthwise_num_outputs = width_multiplier * filters

      
      if conv_with_bias == True:
        depthwise_bias_init = tf.constant_initializer(0.0)
        depthwise_biases = _variable_on_device('depthwise_biases',
                                  [depthwise_num_outputs], depthwise_bias_init, 
                                    trainable=(not freeze))                    
        depthwise_conv = nn.bias_add(depthwise_conv, biases)
    
      if use_batchnorm == True:
        out1 = slim.batch_norm(depthwise_conv, scope='BatchNorm')
        #out1 = self._batch_norm_layer(depthwise_outputs,'BatchNorm')
      else:
        out1 = depthwise_conv
        

      if relu == True:
        depthwise_outputs = activation_fn(out1)
        
      return depthwise_outputs
  ...
  def _mobiledet_quant_act(self,mc,inputs):
